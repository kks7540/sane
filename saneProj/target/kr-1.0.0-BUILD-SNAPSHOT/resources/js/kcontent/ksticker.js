...
var ksticker = new function() {
	...
	that.listDataTable = function(pageNum) {
		...
		that.table = $('#item-list').DataTable({
            ...
            bServerSide: true,
            searching: false,
            sAjaxSource : ctx + '/kcontent/kGeneralStickerList.ajax' + SEARCH_PARAM,
            sServerMethod: 'POST',
            columns: [
              ...
              columnDefs : [
				    ...
				    { orderable : false, targets : 1, render : function ( data, type, row, meta ) {
				    	return '<a href="javascript:void(0);" onclick="ksticker.imagePop(\'' + data + '\');"><img src=\'' + data + '\'/ style="height:40px;"></a>';
				    }},*/
				    { orderable : false, targets : 1, render : function ( data, type, row, meta ) {
				    ...
				    { orderable : false, targets : 6, render : function ( data, type, row, meta ) {
				    	return '<button type="button" class="btn btn-danger" onclick="ksticker.comfNonDisplay(\'' + data + '\');">비전시</button>';
				    }}
				]
